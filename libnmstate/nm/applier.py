...
def _get_ovs_bridge_port_devices(iface_state):
    ...
    ports = []
    ifaces = []
    for port in iface_state.get('bridge', {}).get('port', []):
        ports.append(ovs.PORT_PROFILE_PREFIX + port['name'])
        if port['type'] == 'bond':
            for slave in port.get('link-aggregation', {}).get('slaves', []):
                ifaces.append(slave['name'])
        else:
            ifaces.append(port['name'])
...
def _prepare_proxy_ovs_ifaces(iface_desired_state, new_ifaces_desired_state):
    ...
    if port_desired_state['name'] not in [
        i['name'] for i in new_ifaces_desired_state
    ]:
...
def _create_ovs_port_desired_state(iface_desired_state, bridge_port_state):
    ...
    if 'link-aggregation' in bridge_port_state:
        port_name = bridge_port_state['name']
    else:
        port_name = (
